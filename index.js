...
// 1. Dapatkan referensi ke elemen audio dan tombol
const audio = document.getElementById('myAudio');
const startButton = document.getElementById('startButton');

// 2. Variabel untuk melacak status pemutaran
// Kita mulai dengan status 'false' (audio tidak sedang diputar)
let isPlaying = false; 

// 3. Tambahkan event listener pada tombol pemicu
startButton.addEventListener('click', () => {
    
    // PERIKSA STATUS SAAT INI
    if (isPlaying) {
        // JIKA sedang diputar (isPlaying = true):
        
        // Hentikan audio
        audio.pause(); 
        
        // Opsional: Kembalikan posisi audio ke awal (detik ke-0)
        audio.currentTime = 0; 
        
        // Perbarui status menjadi 'tidak diputar'
        isPlaying = false; 
        
        console.log("Audio dihentikan (Pause).");
        
    } else {
        // JIKA tidak diputar (isPlaying = false):
        
        // Mulai memutar audio
        audio.play()
            .then(() => {
                // Setelah berhasil diputar:
                
                // Perbarui status menjadi 'sedang diputar'
                isPlaying = true; 
                
                console.log("Audio mulai diputar (Play).");
            })
            .catch(error => {
                // Tangani kasus jika browser memblokir pemutaran
